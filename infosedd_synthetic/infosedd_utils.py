...
def get_loss_fn(configs, noise, graph, train, sampling_eps=1e-3):
    ...
    def loss_fn(model, x, y):
        ...
        if configs.is_parametric_marginal and configs.variant == "j":
            ...
            if marginal_flag == 0:
                absorb_indices = list(range(x.shape[1]))
            elif marginal_flag == 1:
                absorb_indices = list(range(x.shape[1], x.shape[1] + y.shape[1]))
            else:
                absorb_indices = None
    ...
    def mutinfo_step_fn(model, x, y, return_noise=False):
        ...
        with torch.no_grad():
            ...
            if config.variant == "j":
                ...
                score_marginal = score_fn(perturbed_batch_marginal, sigma)
                score_conditional = score_fn(perturbed_batch_conditional, sigma)
