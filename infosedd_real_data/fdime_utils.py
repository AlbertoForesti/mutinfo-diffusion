...
def gan_fdime_deranged(D_value_1, D_value_2, device="cpu"):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        BCE = nn.BCELoss()
        D_value_1_float = D_value_1.float()
        D_value_2_float = D_value_2.float()
        try:
            loss_1 = BCE(D_value_1_float, fake_1.float())
        except:
            raise ValueError(f"Invalid shape for D_value_1: {D_value_1.shape}, expected shape: ({batch_size_1}, 1)")
        loss_2 = BCE(D_value_2_float, valid_2.float())
    loss = loss_1 + loss_2
...
class Discriminator(nn.Module):
    ...
    def forward(self, indices, cond=None):
        ...
        if isinstance(self.backbone, transformers.PreTrainedModel):
            if 'hf_dit' in self.config.backbone:
                cond = torch.zeros(indices.shape[0]).to(indices.device)
            last_hidden_state = self.backbone(indices, cond, output_hidden_states=True, return_dict=True).hidden_states[-1]
        else:
            last_hidden_state = self.backbone(indices, cond)
