...
class Accuracy(torchmetrics.aggregation.MeanMetric):
  pass
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    if self.parameterization == 'discriminative':
  ...
  def _init_backbone(self):
    if self.config.backbone == 'dit':
      ...
      self.backbone = models.transformer.Transformer(
        self.config, vocab_size=self.vocab_size)
    elif self.config.backbone == 'unetmlp':
      ...
      if self.config.mode == "train":
        ...
        self.backbone = transformers.AutoModelForMaskedLM.from_pretrained(
        self.config.eval.original_model_path, trust_remote_code=True)
    ...
    elif 'caduceus' in self.config.backbone:
      if self.config.mode == "train":
        self.backbone = transformers.AutoModelForMaskedLM.from_pretrained(
          self.config.eval.checkpoint_path, trust_remote_code=True)
        print(f"Loaded caduceus model from {self.config.eval.checkpoint_path}")
      else:
        self.backbone = transformers.AutoModelForMaskedLM.from_pretrained(
          self.config.eval.original_model_path, trust_remote_code=True)
    else:
    ...
    if self.config.parameterization == 'discriminative':
      self.divergence = self.config.divergence
      self.backbone = fdime_utils.Discriminator(self.backbone, self.config)
      self.alpha = self.config.alpha
      self.backbone = fdime_utils.CombinedNet(
          self.backbone, # Net(length_of_x_plus_y, 1),
  ...
  def _subs_parameterization(self, logits, xt):
    ...
    logits = logits - torch.logsumexp(logits, dim=-1,
                                    keepdim=True)
      # self.analyze_all_tensors_in_memory()
  ...
  def _process_sigma(self, sigma):
    if sigma is None:
      assert self.parameterization == 'ar' or 'caduceus' in self.config.backbone or 'mamba' in self.config.backbone
      return sigma
    if sigma.ndim > 1:
  ...
  def forward(self, x, sigma):
    ...
    if self.config.backbone == 'llada':
      ...
      with torch.cuda.amp.autocast(dtype=torch.float32):
        out = self.backbone(x, self._reshape_based_on_noise(sigma, is_unet_conditioning=True))
        if self.parameterization == 'discriminative':
          return out
        else:
          logits = out.logits
    ...
    # print(f"x: {x[0,:20]}, mask_index: {self.mask_index}")
  ...
  def _compute_loss(self, batch, prefix):
    ...
    if prefix == 'train':
      if self.parameterization != 'discriminative':
        if self.config.training.compute_mutinfo:
          self.train_mutinfo = mutinfo
        self.train_metrics['nll'].update(losses.nlls)
  ...
  @torch.no_grad()
  def _compute_mutinfo(self, x0, prefix):
    ...
    if config.compute_mutinfo:
      mutinfo = self._mutinfo(x0, xt, sigma, dsigma)
    else:
      mutinfo = None
    ...
    if prefix == 'train':
        ...
        if mutinfo is not None:
            self.valid_mutinfo.append(mutinfo)
    else:
  ...
  def _mutinfo(self, x0, xt, sigma, dsigma):
    ...
    if self.parameterization == 'discriminative':
      mask_y = self._get_mask_from_list_of_indices(x0, self.config.mutinfo.var_indices[1])
      y = torch.where(mask_y, x0, torch.zeros_like(x0))  # Keeps original shape, zeros elsewhere
      x = torch.where(~mask_y, x0, torch.zeros_like(x0))  # Keeps original shape, zeros elsewhere
      ...
      mi = torch.mean(torch.log(R)).item()
      return mi
    
    if self.config.variant == 'j':
      ...
      all_var_indices = []
      for i in range(len(self.config.mutinfo.var_indices)):
        all_var_indices += self.config.mutinfo.var_indices[i]
      all_var_indices = torch.tensor(all_var_indices).to(x0.device)
      # raise UserWarning(f"all_var_indices: {all_var_indices}, log_score_marginal: {log_score_marginal.shape}, log_score_joint: {log_score_joint.shape}")
      log_score_marginal = log_score_marginal[:,all_var_indices]
      log_score_joint = log_score_joint[:,all_var_indices]
      xt = xt[:,all_var_indices]
      ...
      raise NotImplementedError(f"Unknown variant: {self.config.variant}")
  ...
  def on_validation_epoch_start(self):
    torch.cuda.empty_cache()
    if hasattr(self, 'valid_mutinfo'):
      self._old_valid_mutinfo = self.valid_mutinfo
    else:
      self._old_valid_mutinfo = None
    self.valid_mutinfo = []
    if self.ema:
  ...
  def validation_step(self, batch, batch_idx):
    ...
    mutinfo = self._compute_mutinfo(batch['input_ids'], prefix='val')
    return {
      'loss': loss,
      'mutinfo': mutinfo
    }
  ...
  def on_validation_epoch_end(self):
    ...
    if self.config.eval.compute_mutinfo:
      self.valid_mutinfo_std = np.std(self.valid_mutinfo)
      self.valid_mutinfo = np.mean(self.valid_mutinfo)
      self.log('val/mutinfo',
                self.valid_mutinfo,
                on_epoch=True,
                on_step=False,
                sync_dist=True)
      if np.isnan(self.valid_mutinfo):
        self.valid_mutinfo = self._old_valid_mutinfo
    if self.ema:
      self.ema.restore(self.get_params())
    torch.cuda.empty_cache()
    # Memory consumption check
  
  def print_is_grad_for_backbone(self):
  ...
  def get_score(self, x, sigma, return_logscore=False):
    ...
    if self.parameterization == 'subs':
      ...
      masked_score = model_output + log_k
  ...
  def _discriminative_loss(self, x0):
    mask_y = self._get_mask_from_list_of_indices(x0, self.config.mutinfo.var_indices[1])
    y = torch.where(mask_y, x0, torch.zeros_like(x0))  # Keeps original shape, zeros elsewhere
    x = torch.where(~mask_y, x0, torch.zeros_like(x0))  # Keeps original shape, zeros elsewhere
    y_shuffle = torch.index_select(y, 0, fdime_utils.derangement(list(range(y.shape[0])), self.device))
  ...
  def _reshape_based_on_noise(self, x, is_unet_conditioning=False):
    ...
    if self.config.backbone == 'hf_dit' and is_unet_conditioning:
      return x
    if 'caduceus' in self.config.backbone or 'mamba' in self.config.backbone:
      if is_unet_conditioning:
        return None
      else:
        return x[:, None]
    return x[:, None]
  ...
  def _loss(self, x0, attention_mask):
    ...
    elif self.parameterization == 'discriminative':
      assert not self.config.train_marginal, \
        "discriminative loss does not support train_marginal, this flag does not affect training but you should not set it to True"
      return self._discriminative_loss(x0)
    else:
      loss = self._forward_pass_diffusion(input_tokens)

    if self.config.train_marginal and self.selected_mask is not None and self.selected_mask < len(self.config.mutinfo.var_indices):
      am_mask = self._get_mask_from_list_of_indices(
        attention_mask, self.config.mutinfo.var_indices[self.selected_mask])
      attention_mask = torch.where(
        am_mask, attention_mask, torch.zeros_like(attention_mask))
  ...
  def get_params(self):
    model_params = self.backbone.parameters()
    
    return list(itertools.chain(model_params,
